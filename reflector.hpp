...
public:
   ...
   * the file (and thus the reflector) is valid
  ...
  int config(string config_file_path);
   ...
   /*
   * Description: convert takes in a pointer to an integer representing a 
   * character (zero based, ie. 0=A, 1=B, ...) and switches the value of this 
   * character with the character the original input character is mapped to in 
   * the reflector config file 
