...
int Plugboard::config(string config_file_path) {
  ...
  Processor* plugboard_processor =
    new Processor(Processor::file_type::plugboard);
  ...
  while(plugboard_processor->good()) {
    ...
    error_code = plugboard_processor->exists_within(config_file_integer,
   ...
   error_code = plugboard_processor
